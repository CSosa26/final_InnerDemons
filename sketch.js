...
function draw() {
  ...
  if (screen == 0) {    // insert main screen code here, replace below
    background(100);
    textSize(200);
    textAlign(CENTER);
    text('MAIN MENU',600, 300);

    if (keyIsDown(32)) { //space bar
      screen = 3; // Change number to 1 after input character and stage selection screen
    }
...
if (screen == 2) { // insert stage selection screen code here
  ...
  if (screen == 4) { // remove code, insert K.O gif below
    textSize(200);
    textAlign(CENTER);
    text('KO',600, 300);
